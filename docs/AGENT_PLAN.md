...
- UI CRUD (§6.5)
- Привязка пула услуг к User (на карточке `/users/:id`; отдельный UI `/employee_skills` снят)
...
- Оплата труда техников / начисления (§7.2) — только `completed` по `completed_at`; откат убирает из начислений; фильтры сотрудник/выплата/услуга
- Неоплаченные заказчиком / долги (§7.3) + ссылка на «Оплаты заказчиков»
- Воронка (§7.4), по заказчикам (§7.5), по услугам (§7.6)

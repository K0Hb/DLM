...
Admin MUST see a Russian-language report of current work-order counts by status, plus active service lines (`assigned` / `in_progress`, not soft-removed) grouped by assignee. Soft-removed lines MUST NOT appear in the in-hand section.
...
Admin MUST see a per-customer summary for work orders created in a date range: order count, sum of amounts due (`customer_payment_amount` when unpaid / paid amount when paid as displayed consistently), amount already paid, and remaining debt (unpaid orders). UI MUST be in Russian.
...
Admin MUST see a per-service summary of completed lines with `completed_at` in the period: quantity sum and accrued amount (`quantity * technician_price_snapshot`). Soft-removed completed lines MUST still count (same rule as payroll accruals). UI MUST be in Russian.
...
Payroll MUST sum `quantity * technician_price_snapshot` for lines with `status=completed` and `completed_at` in the period, grouped by assignee, with line detail. Admin MUST be able to filter by assignee, technician-paid flag, and service. Rolled-back completed lines MUST NOT appear.

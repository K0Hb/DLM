...
![Демо интерфейса DLM](docs/demo/dlm-demo.gif)

Короткая прогулка по интерфейсу
...
DLM рассчитан прежде всего на работу **в самой лаборатории**: программа крутится на одном компьютере, а сотрудники открывают её в браузере — с рабочих мест и с телефона в той же Wi‑Fi-сети.

Так можно обойтись без аренды сервера в интернете (и без оплаты «облака», внешнего адреса и домена). Наряды, оплаты и фото остаются у вас на диске: данные не уезжают на чужой хостинг, проще делать копии и ничего не зависит от стороннего провайдера.

Инструкция ниже — для Windows 11 с WSL2: как один раз настроить доступ по локальной сети. В интернет приложение при этом не выкладывается.
